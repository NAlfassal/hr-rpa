...
${FINAL_REPORT_DATE}    2025-11-2  # تاريخ إرسال التقرير النهائي
#${FORM_TARGET_TIME}     09:00:00    # وقت إرسال الفورم الأولي
${REMINDER_TIME}        8:42:00    # وقت إرسال التذكيرات
#${REPORT_TARGET_TIME}   11:00:00    # وقت إرسال التقرير النهائي

*** Tasks ***
Execute Full Automation Cycle
    Log To Console    Automation started.

    # 1. إرسال الفورم في بداية الشهر (أو في أي وقت تحدده)
    Wait For Specific Time    ${FINAL_REPORT_DATE} ${FORM_TARGET_TIME}
    Send Form Link To Employees

    # 2. حساب تاريخ التذكير وإرسال التذكيرات
    ${reminder_date}=    Subtract Time From Date    ${FINAL_REPORT_DATE}    2 days    result_format=%Y-%m-%d
    Wait For Specific Time    ${reminder_date} ${REMINDER_TIME}
    Send Reminders To Non-Responders

    # 3. إرسال التقرير النهائي في الموعد المحدد
